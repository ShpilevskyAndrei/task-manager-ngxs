...
export class AuthService {
  ...
  public login(
    ...
    return this._httpService
      .post<IUser[]>(API, ENDPOINTS.auth['login'], [])
      .pipe(
        map((users: IUser[]): IResponse<ITokens> => {
          ...
          if (
            ...
            this._refreshTokenStorageService.setItem(
  ...
  public logout(): Observable<IResponse<boolean>> {
    return this._httpService.post(API, ENDPOINTS.auth['logout'], {}).pipe(
      map((): IResponse<boolean> => {
        const response: IResponse<boolean> =
          this._authServiceHelper.logoutControl();

        if (response.status === ResponseStatusesEnum.Error && !response.data) {
          return response;
        }
